import dg535
import ks33600a
import generate_arb_ks33600a

EXCITATION = 5e-6
PI = 60e-9
PI_2 = PI / 2
READOUT = 300e-9
AOM_DELAY = 1e-6
TAU = 60e-9
MICROWAVE = PI + TAU + PI_2 + TAU + PI
SINGLE_CYCLE = AOM_DELAY + EXCITATION + MICROWAVE + AOM_DELAY + READOUT
# does it take extra time to save to memory?

def prepare_files():
    generate_arb_ks33600a.generate("waveforms/test.csv", 77e6, [0, EXCITATION, AOM_DELAY + EXCITATION + MICROWAVE, \
    AOM_DELAY + EXCITATION + MICROWAVE + READOUT], 10e-6)

if __name__ == "__main__":
    generate = True
    if generate == True:
        prepare_files()
    # note there is a settling time of 2 seconds
    dg = dg535.DG535("GPIB0::15::INSTR")

    dg.configure_sequence(
        t0_to_a=AOM_DELAY + EXCITATION,
        a_to_b=1e-6, # longer pulse to trigger on
        t_cycle=SINGLE_CYCLE * 2
    )
    dg.close()

    # trigger on T0
    ks = ks33600a.KS33600A("USB0::0x0957::0x5707::MY53800810::INSTR")
    ks.upload_csv("waveforms/test.csv")
    ks.run(vpp=1)
    # measured on scope: seems about 85% of specified peak-to-peak gets through, ex. 100 mV => 85 mV
    ks.close()




