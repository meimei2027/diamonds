...
to applied current) for NV alignment/biasing -- chosen over the SPD1168X for
this role because of its higher current limit.
...
class SPD1305X:
    ...
    def set_field(self, field_g, voltage_margin=1.2):
        ...
        """
        if field_g < 0:
            raise ValueError("SPD1305X: set_field() only supports field_g >= 0 "
        ...
        print(f"SPD1305X: field set to {field_g} G (I={current_a:.4f} A, "
              f"V={voltage_v:.4f} V)")
